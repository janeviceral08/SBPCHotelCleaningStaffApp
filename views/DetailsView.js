...
export function DetailsView({ navigation, route }) {
  const { checkin, editCheckin, editpenalty, checkout, balance, editCheck_in, editCheckout, editCheckinExtension, Order, tasks, rooms, changeRoom, GoRefund  } = useTasks();
...
const print = () => {
 ...
 useEffect(() => {
            ...
            0  
            :  (parseFloat(checkinInfo.price)*Math.floor(sub/(1000*60 * 60 * 24))/ parseFloat(checkinInfo.no_person)*parseFloat(checkinInfo.no_person_discount))/100*parseFloat(checkinInfo.discount))
...
<DateTimePickerModal
      ...
      />
<TouchableOpacity onPress={showDatePicker_checkout} style={{marginLeft: 50}}>
<Body>
