...
export function HourCheckinSheet({ navigation, route }) {
...
const print = () => {
 ...
 useEffect(() => {
  ...
  let minutes_extension = parseInt(duration_extension.asMinutes())-hours_extension*60;     
         let in_check_extension =  moment(checkinInfo.check_in * 1000).format('MMMM D, YYYY')
  ...
  let stay = parseFloat(checkinInfo.price)*1
  setstay_total(stay)
  setdiscount(checkinInfo.discount == ''||checkinInfo.discount == undefined ||checkinInfo.discount == 0 ||checkinInfo.discount == '0'?
  0  
  :  (parseFloat(checkinInfo.price)* 1 / parseFloat(checkinInfo.no_person)*parseFloat(checkinInfo.no_person_discount))/100*parseFloat(checkinInfo.discount))
  setextension_calcu(checkinInfo.extension_rate == 0 || checkinInfo.extension_rate == '' || checkinInfo.extension_rate == undefined? 0
  : parseFloat(checkinInfo.extension_rate))
setpenalty(checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined? 0: parseFloat(checkinInfo.penalty))
setextra(checkinInfo.extra_person=="" || checkinInfo.extra_person == undefined? 0: parseFloat(checkinInfo.extra_person)* parseFloat(checkinInfo.extra_amount)*1 )
...
const time_checkin = moment(parseFloat(checkinInfo.check_in*1000)).format('YYYY-MM-DD')+'T'+moment(parseFloat(checkinInfo.check_in*1000)).format('hh:mm')+':00.000Z'

const add = moment(time_checkin).add(parseFloat(hour_duration), 'hours')
...
<Text style={styles.textTitle}>E-mail</Text>
 ...
 <ScrollView style={{height: "80%"}}>
      <View>
                    ...
                    <TextInput
                        style={[styles.textInputTitle, {borderColor: Colors.buttons,borderWidth: 1,borderRadius: 5,  }]}
                        onChangeText={(text) => setExtAmount(text)}
                        autoCorrect={true}
                        value={ExtAmount}
                    />
...
<Right>
<MaterialCommunityIcons  name="printer-wireless" size={25} color={"black"} onPress={print}/>
</Right>
...
<Collapse>
        ...
        <Text style={{  fontSize: 15,fontWeight: 'bold'}}>
         T O T A L:
          ...
          {currencyFormat(parseFloat(Order&& Order.length ?Order.reduce((sum, i) => ( checkinInfo.temp_id == i.customer_id && i.method != "Paid"?
                              ...
                              sum += 0
														), 0) : 0)+parseFloat(extension_calcu)+parseFloat(checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined? 0: checkinInfo.penalty)+parseFloat(extra)+parseFloat(stay_total)-parseFloat(discount))}
    </Text>     
      </Body>   
    </CardItem>
      </CollapseHeader>
      <CollapseBody>
        <ListItem>
        <Body>
        <Text style={{  fontSize: 11,fontWeight: '900'}}>{checkinInfo.hour_duration} Hours Stay: </Text>
            </Body>
            <Right>
        ...
        <Text style={{  fontSize: 13,fontWeight: 'bold'}}>B A L A N C E: </Text>
          ...
          currencyFormat(parseFloat(Order&& Order.length ?Order.reduce((sum, i) => ( checkinInfo.temp_id == i.customer_id && i.method != "Paid"?
            sum += (i.quantity * i.price):
            sum += 0
          ), 0) : 0)+parseFloat(extension_calcu)+parseFloat(checkinInfo.penalty== '' ||  checkinInfo.penalty == null||  checkinInfo.penalty == undefined? 0: checkinInfo.penalty)+parseFloat(extra)+parseFloat(stay_total)-parseFloat(discount)-parseFloat(checkinInfo.payment))
        } </Text>
              </Right>
        </ListItem>
        <ListItem last onPress={()=>setvisibleRefund(true)}>
        <Body>
        <Text style={{  fontSize: 13,fontWeight: 'bold'}}>R E F U N D </Text>
            </Body>
        ...
        <Modal visible={visibleRefund}
                            ...
                            <AntDesign name="closecircleo" size={20} color={"black"} />
                                                ...
                                                <Text style={styles.textTitle}>Amount Refunded:  {currencyFormat(parseFloat(Refunded))}</Text>
                                                        <TextInput
                                                           style={{ height: 40, borderBottomColor: 'gray',borderWidth: 1,borderRadius: 5, paddingLeft: 20, width: 250, marginBottom: 10 }}
                                                  ...
                                                  onPress={() => {
                                                    ...
                                                    Alert.alert(
                                                      ...
                                                      [
                                                        ...
                                                        { text: "OK", onPress: () =>  { Toast.show('PROCESSING...'); setvisibleRefund(false);BackPage();GoRefund(checkinInfo, Refunded)
                                                      }         
                                                  }
                                                  ],
                                                  { cancelable: false }
            ...
            sum += 0
                          ...
                          <Text style={styles.textTitle}>  Amount Payed: {currencyFormat(parseFloat(checkinInfo.payment))}</Text>
                                                ...
                                                <TouchableOpacity
                                                  ...
                                                  onPress={() => {checkout(stay_total,discount,extension_calcu,extra,DaysValue,checkinInfo); BackPage()}}
                                                >
