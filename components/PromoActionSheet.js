...
export function PromoActionSheet({ visible, closeOverlay, roomTypeInfo }) {
  const { tasks,rooms,goods, createTask, PromoCheckin, Booking } = useTasks();
  ...
  const handleConfirm = (date) => {
    ...
    if(roomTypeInfo.duration_mode == "Hour"){
      var returned_endate = moment(date).add(parseInt(roomTypeInfo.promo_duration), 'hours');  
    ...
    else if (roomTypeInfo.duration_mode == "Daily"){
      var returned_endate = moment(date).add(parseInt(roomTypeInfo.promo_duration), 'days');  
      setCheckout(returned_endate)
      hideDatePicker();
                const newdar = Booking.filter(x =>  x.in_check >= moment(date).unix()&& x.in_check <= moment(returned_endate).unix() && x.status =='Confirmed'&& x.room ==roomTypeInfo.temp_id)
...
if( check_in == "" && Customer == "" && Address == "" || Contact == ""  || Nationality == "" || NoPerson == "" ||  RoomID == "" ){
  ...
  PromoCheckin(newRoomType,newRoomPrice,newMaxPerson,id,RoomID,RoomFloor,RoomName,check_in,check_out,Company,Customer,Address,Contact,Nationality,Email,NoPerson,ExtraPerson,PersonWDiscount,Discount,DiscountID,Note,newExtraPerson,control_num, roomTypeInfo,RoomTypeIds,"","Over The Counter");
...
:null}
       ...
       <Text>Room Price</Text>
               <TextInput
                        ...
                        autoFocus={true}
                        value={currencyFormat(parseFloat(newRoomPrice))}
                        editable = {false}
                    />
