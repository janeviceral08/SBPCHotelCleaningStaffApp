...
export function ActionSheet({ visible, closeOverlay, roomTypeInfo }) {
  const { tasks,rooms,goods, createTask, createCheckin, Booking } = useTasks();
  ...
  const handleConfirm_checkout = (date) => {
    ...
    const newdar = Booking.filter(x =>  x.in_check >= moment(check_in).unix()&& x.in_check <= moment(date).unix() && x.status =='Confirmed'&& x.room ==roomTypeInfo.temp_id)
  ...
  const GoToIn = () =>{
     if( check_in == "" && Customer == "" && Address == "" || Contact == ""  || Nationality == "" || NoPerson == "" ||  RoomID == "" ){
      ...
      createCheckin(newRoomType,newRoomPrice,newMaxPerson,id,RoomID,RoomFloor,RoomName,check_in,check_out,Company,Customer,Address,Contact,Nationality,Email,NoPerson,ExtraPerson,PersonWDiscount,Discount,DiscountID,Note,newExtraPerson,control_num, roomTypeInfo, RoomTypeIds, RoomIDTemp,"","Over The Counter");
  ...
  return (
    ...
    >
     ...
     <Button title={check_out == ""?"Show Date Picker": moment(check_out).format('MMMM D, YYYY hh:mm a')} onPress={showDatePicker_checkout} color={Colors.buttons}/>
      <DateTimePickerModal
